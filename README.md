...
| `dirNotExists` / `fileNotExists` | list of relative paths | Every path must be absent (AND) |
...
ripple list --dir-not-exists ios/Pods
ripple list --file-not-exists ios/Podfile
...
| `--dir-not-exists <path>` | Only packages that do not contain this relative directory (repeatable, AND). |
| `--file-not-exists <path>` | Only packages that do not contain this relative file (repeatable, AND). |
...
- **`run:`** — runs once with cwd = the Ripple root (all list steps in order).
  ...
  `--file-exists`, `--dir-not-exists`, `--file-not-exists`, `--depends-on`,
